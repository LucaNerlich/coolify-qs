...
bin="$repo_root/omarchy/bin/coolify-qs"
expected_file="$repo_root/omarchy/bin/coolify-qs.sha256"
srcid_file="$repo_root/omarchy/bin/coolify-qs.srcid"
...
and commit omarchy/bin/coolify-qs, .sha256, and .srcid in the same
...
if ! nm "$bin" | grep 'coolify_qs' >/dev/null; then
...
if [[ "${VERIFY_BUNDLE_SKIP_REBUILD:-}" == 1 ]]; then
  echo "verified: omarchy/bin/coolify-qs is non-stripped, version $crate_version, hash $committed, source id $actual_srcid (rebuild skipped)"
  exit 0
fi
...
actual="$(sha256sum "$tmp/target/$target/release/coolify-qs" | awk '{print $1}')"
...
echo "verified: omarchy/bin/coolify-qs is non-stripped, version $crate_version, and matches the reproducible build ($actual)"
